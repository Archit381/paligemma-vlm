...
class SiglipVisionConfig:
    ...
    def __init__(
        self,
        hidden_size = 768,                  # Size of the embeddings vector
...
class SiglipAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if attn_weights.size() != (batch_size, self.num_heads, seq_length, self.head_dim):
            raise ValueError(
                f"Attention weights should be of size {(batch_size, self.num_heads, seq_length, self.head_dim)}, but is "
                f"{attn_weights.size()}"
            )
...
class SiglipVisionModel(nn.Module):
    ...
    def forward(self, pixel_values) -> Tuple:
        ...
        return self.vision_model(pixel_values = pixel_values)     ## Takes in batch of images and gives embedding vector for each patch
