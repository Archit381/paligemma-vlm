...
class GemmaRMSNorm(nn.Module):
    ...
    def _norm(self, x):
        return x*torch.sqrt(x.pow(2).mean(-1, keep_dim=True) + self.eps)
        ...
        def forward(
            ...
            **kwargs,
            ...
            if attn_output.size() != (bsz, self.num_heads, q_len, self.head_dim):
                raise ValueError(
                    f"attn_output should be of the size {(bsz, self.num_heads, q_len, self.head_dim)}, but is"
                    f"{attn_output.size()}" 
                )
...
class PaliGemmaForConditionalGeneration(nn.Module):
    def __init__(self, config: PaliGemmaConfig):
        ...
        self.multi_tower_projector = PaliGemmaMultiModalProjector(config)
        self.vocab_size = config.vocab_size

        language_model = GemmaForCasualLM(config.text_config)
        self.language_model = language_model

        self.pad_token_id = self.confg.pad_token_id if self.config.pad_token_id is not None else -1
    ...
    def forward(
        self,
        input_embeds: torch.LongTensor = None,
        ...
        image_features = self.multi_tower_projector(selected_image_features)
